...
function createFoldersBatch(isSilent, force) {
  ...
  for (var r = CONFIG.START_ROW; r <= lastRow; r += blockHeight) {
    ...
    try {
      ...
      var headerUrlCell = sheet.getRange(r, urlCol);
      var headerUrl = (headerUrlCell.getDisplayValue() || "").toString().trim();
      if (!headerUrl) {
        headerUrl = getUrlFromCell_(headerUrlCell);
      }
      if (!headerUrl) {
        headerUrlCell.setValue(projectFolder.getUrl());
      }
